...
use nps_nip::acme::messages::{
    Authorization, ChallengeRespondPayload, Directory, Identifier, NewAccountPayload,
    NewOrderPayload, Order, ProblemDetail,
};
use nps_nip::acme::wire;
use nps_nip::acme::{jws, AcmeClient, AcmeServer, AcmeServerOptions};
...
async fn issue_agent_cert_round_trip_returns_valid_pem_chain() {
    ...
    let pem = client
        .issue_agent_cert(&fx.agent_nid)
        .await
        .expect("issue_agent_cert");
    assert!(
        pem.contains("BEGIN CERTIFICATE"),
        "PEM missing certificate marker"
    );
...
async fn respond_agent01_tampered_signature_server_returns_challenge_failed() {
    ...
    let acct_env = jws::sign(
        &jws::ProtectedHeader {
            alg: jws::ALG_EDDSA.into(),
            nonce: nonce.clone(),
            url: dir.new_account.clone(),
            jwk: Some(jwk),
            kid: None,
        },
        Some(&NewAccountPayload {
    ...
    let order_env = jws::sign(
        &jws::ProtectedHeader {
            alg: jws::ALG_EDDSA.into(),
            nonce: nonce.clone(),
            url: dir.new_order.clone(),
            jwk: None,
            kid: Some(account_url.clone()),
        },
    ...
    let authz_env = jws::sign::<NewAccountPayload>(
        &jws::ProtectedHeader {
            alg: jws::ALG_EDDSA.into(),
            nonce: nonce.clone(),
            url: order.authorizations[0].clone(),
            jwk: None,
            kid: Some(account_url.clone()),
        },
        None,
...
fn parse_pem_chain_b64u(pem: &str) -> Vec<String> {
    ...
    while let Some(start) = pem[pos..].find(marker_begin).map(|i| pos + i) {
        ...
        let Some(end) = pem[body_start..].find(marker_end).map(|i| body_start + i) else {
            break;
        };
