...
use nps_nip::{NipCaClient, NipCaCrl};
...
fn router_register_and_verify_and_crl() {
    ...
    let typed_crl: NipCaCrl = serde_json::from_value(crl_body).unwrap();
    assert!(NipCaClient::verify_crl_signature(
        &typed_crl,
        &ca.get_ca_public_key()
    ));

    let certificates = router.handle(&CaRequest::new("GET", "/v1/certificates"));
    assert_eq!(certificates.status, 200);
    let entries = certificates.json_value().unwrap()["entries"]
        .as_array()
        .unwrap()
        .clone();
    assert_eq!(entries.len(), 1);
    assert_eq!(entries[0]["nid"], json!(nid));
}
...
fn router_operator_auth_gate() {
    ...
    assert_eq!(
        router
            .handle(&CaRequest::new("GET", "/v1/certificates"))
            .status,
        401
    );
    ...
    let listed = router.handle(
        &CaRequest::new("GET", "/v1/certificates")
            .with_header("Authorization", "Bearer secret-key"),
    );
    assert_eq!(listed.status, 200);
    assert_eq!(
        listed.json_value().unwrap()["entries"]
            .as_array()
            .unwrap()
            .len(),
        1
