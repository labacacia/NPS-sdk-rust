...
async fn register_x509_round_trip_verifier_accepts() {
    ...
    let frame = build_v2_frame(
        agent_nid,
        agent_sk.verifying_key().as_bytes(),
        &ca_sk,
        Some(ATTESTED),
        leaf.der(),
        root.der(),
...
async fn register_x509_leaf_eku_stripped_rejects_eku_missing() {
    ...
    let root = mk_root(&ca_sk, ca_nid, &[1]);
    let tampered_der = build_leaf_without_eku(
        agent_nid,
        agent_sk.verifying_key().as_bytes(),
        &ca_sk,
        &root,
    ...
    let frame = build_v2_frame_with_der(
        agent_nid,
        agent_sk.verifying_key().as_bytes(),
        &ca_sk,
        None,
    ...
    assert_eq!(
        r.error_code,
        Some(error_codes::CERT_EKU_MISSING),
        "got code={:?} msg={:?}",
        r.error_code,
        r.message
    );
...
async fn register_x509_leaf_for_different_nid_rejects_subject_mismatch() {
    ...
    assert_eq!(
        r.error_code,
        Some(error_codes::CERT_SUBJECT_NID_MISMATCH),
        "got code={:?} msg={:?}",
        r.error_code,
        r.message
    );
...
async fn v2_verifier_rejects_v2_frame_when_trusted_roots_missing() {
    ...
    assert_eq!(
        r.error_code,
        Some(error_codes::CERT_FORMAT_INVALID),
        "got code={:?} msg={:?}",
        r.error_code,
        r.message
    );
...
fn build_leaf_without_eku(
    ...
    let cert = params
        .signed_by(&subject_spki, ca_root, &ca_keypair)
        .expect("signed_by");
    cert.der().to_vec()
}
