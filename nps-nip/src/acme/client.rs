...
impl AcmeClient {
    ...
    async fn ensure_directory(&mut self) -> Result<(), String> {
        ...
        let dir: Directory = resp
            .json()
            .await
            .map_err(|e| format!("decode directory: {e}"))?;
    ...
    async fn new_order(&mut self, nid: &str) -> Result<Order, String> {
        ...
        let order: Order = resp
            .json()
            .await
            .map_err(|e| format!("decode order: {e}"))?;
        Ok(order)
    }
    ...
    async fn respond_agent_01(&mut self, authz: &Authorization) -> Result<(), String> {
        ...
        let env = jws::sign(
            ...
            Some(&ChallengeRespondPayload {
                agent_signature: jws::b64u_encode(&agent_sig.to_bytes()),
            }),
            &self.signing_key,
        )?;
    ...
    async fn finalize_order(&mut self, order: &Order, nid: &str) -> Result<Order, String> {
        ...
        let env = jws::sign(
            ...
            Some(&FinalizePayload {
                csr: jws::b64u_encode(&csr_der),
            }),
            &self.signing_key,
        )?;
        ...
        resp.json()
            .await
            .map_err(|e| format!("decode finalized order: {e}"))
    }
    ...
    fn build_csr(&self, nid: &str) -> Result<Vec<u8>, String> {
        let key_pair =
            dalek_to_rcgen_keypair(&self.signing_key).map_err(|e| format!("dalek→rcgen: {e}"))?;
