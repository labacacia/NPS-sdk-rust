...
use super::jws::{self, Envelope, Jwk, ProtectedHeader};
use super::messages::*;
use super::wire;
...
fn handle_request(
    ...
    let (status, body, extra_headers) = match result {
        ...
        Err((status, body)) => (
            status,
            body,
            vec![("Content-Type".into(), wire::CONTENT_TYPE_PROBLEM.into())],
        ),
    };
...
fn handle_new_account(
    ...
    let pub_key = jws::public_key_from_jwk(jwk).map_err(|e| {
        problem(
            400,
            "urn:ietf:params:acme:error:malformed",
            &format!("jwk parse: {e}"),
    ...
    state
        ...
        .unwrap()
        .account_jwks
        .insert(account_url.clone(), jwk.clone());

    let new_nonce = mint_nonce(state);
    Ok(json_response(
        201,
        &serde_json::json!({"status": wire::STATUS_VALID}),
        vec![
...
fn handle_new_order(
    ...
    if !verify_account(state, &env, &header) {
        return Err(problem(
            ...
            &format!(
                "unknown kid: {}",
                header.kid.as_deref().unwrap_or("<missing>")
            ),
        ));
    }
    ...
    let account_url = header.kid.unwrap_or_default();
    {
        ...
        s.challenges.insert(
            ...
            ChallengeState {
                id: chall_id.clone(),
                type_: wire::CHALLENGE_AGENT_01.into(),
                status: wire::STATUS_PENDING.into(),
                token,
...
fn handle_authz(
    ...
    let (az, challenges) = {
        ...
        let challenges: Vec<Challenge> = az
            ...
            .map(|cs| Challenge {
                type_: cs.type_,
                url: format!("{base_url}/chall/{}", cs.id),
                status: cs.status,
                token: cs.token,
                validated: None,
                error: None,
...
fn handle_challenge(
    ...
    let agent_sig_b64 = match payload.and_then(|p| Some(p.agent_signature)) {
        ...
        _ => {
            ...
            return Err(problem(
                400,
                error_codes::ACME_CHALLENGE_FAILED,
    ...
    Ok(json_response(
        ...
        &Challenge {
            type_: ch.type_,
            url: format!("{base_url}/chall/{}", ch.id),
            status: ch.status,
            token: ch.token,
            validated: None,
            error: None,
...
fn handle_finalize(
    ...
    let csr_der = jws::b64u_decode(&csr_b64).map_err(|e| {
        problem(
            400,
    ...
    let (_rem, csr) = X509CertificationRequest::from_der(&csr_der).map_err(|e| {
        problem(
            400,
            "urn:ietf:params:acme:error:badCSR",
            &format!("CSR parse: {e}"),
    ...
    if subject_cn != Some(os.identifier.value.as_str()) {
        return Err(problem(
            400,
            error_codes::CERT_SUBJECT_NID_MISMATCH,
            &format!(
                "CSR subject CN '{}' does not match order identifier '{}'",
                subject_cn.unwrap_or(""),
                os.identifier.value
            ),
        ));
    }
    let pub_key_bytes = csr
        .certification_request_info
        .subject_pki
        .subject_public_key
        .data
    ...
    let _ = VerifyingKey::from_bytes(&subject_pub).map_err(|e| {
        problem(
            400,
            "urn:ietf:params:acme:error:badCSR",
            &format!("CSR pubkey: {e}"),
    ...
    Ok(json_response(
        ...
        &Order {
            ...
            authorizations: vec![authz_url],
            finalize: os.finalize_url,
            certificate: os.certificate_url,
            error: None,
        },
        vec![("Replay-Nonce".into(), new_nonce)],
    ))
...
fn read_envelope(req: &mut Request) -> Result<Envelope, (u16, String)> {
    ...
    req.as_reader().read_to_string(&mut body).map_err(|e| {
        problem(
            400,
            "urn:ietf:params:acme:error:malformed",
            &format!("body read: {e}"),
...
fn parse_header(env: &Envelope) -> Result<ProtectedHeader, (u16, String)> {
    let bytes = jws::b64u_decode(&env.protected).map_err(|e| {
        problem(
            400,
            "urn:ietf:params:acme:error:malformed",
...
fn verify_account(state: &Arc<Mutex<State>>, env: &Envelope, header: &ProtectedHeader) -> bool {
    ...
    let Some(jwk) = state.lock().unwrap().account_jwks.get(kid).cloned() else {
        return false;
    };
    let Ok(pk) = jws::public_key_from_jwk(&jwk) else {
        return false;
    };
