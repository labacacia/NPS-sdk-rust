...
mod tests {
    ...
    fn cert_verification_errors() {
        ...
        assert_eq!(to_nps_status(CERT_SIGNATURE_INVALID),  "NPS-AUTH-UNAUTHENTICATED");
        assert_eq!(to_nps_status(CERT_UNTRUSTED_ISSUER),   "NPS-AUTH-UNAUTHENTICATED");
    ...
    fn ca_service_errors() {
        ...
        assert_eq!(to_nps_status(CA_SCOPE_EXPANSION_DENIED),"NPS-AUTH-FORBIDDEN");
    }

    #[test]
    fn ocsp_errors() {
        assert_eq!(to_nps_status(OCSP_UNAVAILABLE),    "NPS-SERVER-UNAVAILABLE");
        assert_eq!(to_nps_status(OCSP_STAPLE_EXPIRED), "NPS-AUTH-UNAUTHENTICATED");
    }

    #[test]
    fn trust_frame_errors() {
        assert_eq!(to_nps_status(TRUST_FRAME_INVALID),               "NPS-CLIENT-BAD-FRAME");
        assert_eq!(to_nps_status(TRUST_FRAME_EXPIRED),               "NPS-AUTH-UNAUTHENTICATED");
        assert_eq!(to_nps_status(TRUST_FRAME_GRANTOR_REVOKED),       "NPS-AUTH-UNAUTHENTICATED");
        assert_eq!(to_nps_status(TRUST_FRAME_SCOPE_EXCEEDS_GRANTOR), "NPS-AUTH-FORBIDDEN");
        assert_eq!(to_nps_status(TRUST_FRAME_NODES_PATTERN_INVALID), "NPS-CLIENT-BAD-FRAME");
    }

    #[test]
    fn revoke_frame_errors() {
        assert_eq!(to_nps_status(REVOKE_FRAME_INVALID),             "NPS-CLIENT-BAD-FRAME");
        assert_eq!(to_nps_status(REVOKE_FRAME_UNAUTHORIZED_ISSUER), "NPS-AUTH-FORBIDDEN");
        assert_eq!(to_nps_status(REVOKE_FRAME_SERIAL_MISMATCH),     "NPS-CLIENT-BAD-PARAM");
        assert_eq!(to_nps_status(REVOKE_FRAME_REASON_UNKNOWN),      "NPS-CLIENT-BAD-FRAME");
    }
    ...
    fn assurance_and_reputation_errors() {
        ...
        assert_eq!(to_nps_status(REPUTATION_ENTRY_INVALID),     "NPS-CLIENT-BAD-FRAME");
        assert_eq!(to_nps_status(REPUTATION_LOG_UNREACHABLE),   "NPS-DOWNSTREAM-UNAVAILABLE");
        assert_eq!(to_nps_status(REPUTATION_GOSSIP_FORK),       "NPS-SERVER-INTERNAL");
        assert_eq!(to_nps_status(REPUTATION_GOSSIP_SIG_INVALID),"NPS-CLIENT-BAD-FRAME");
    }
    ...
    fn group_session_nid_errors() {
        ...
        assert_eq!(to_nps_status(CA_SESSION_VALIDITY_INVALID), "NPS-CLIENT-BAD-PARAM");
        ...
        assert_eq!(to_nps_status(CERT_PARENT_REVOKED),         "NPS-AUTH-UNAUTHENTICATED");
    }
    ...
    fn x509_acme_errors() {
        ...
        assert_eq!(to_nps_status(CERT_SUBJECT_NID_MISMATCH), "NPS-CLIENT-BAD-FRAME");
    ...
    fn constants_have_correct_prefix() {
        let codes = [
            CERT_EXPIRED, CERT_REVOKED, CERT_SIGNATURE_INVALID, CERT_UNTRUSTED_ISSUER,
            CERT_CAPABILITY_MISSING, CERT_SCOPE_VIOLATION,
            CA_NID_NOT_FOUND, CA_NID_ALREADY_EXISTS, CA_SERIAL_DUPLICATE,
            CA_RENEWAL_TOO_EARLY, CA_SCOPE_EXPANSION_DENIED,
            OCSP_UNAVAILABLE, OCSP_STAPLE_EXPIRED,
            TRUST_FRAME_INVALID, TRUST_FRAME_EXPIRED, TRUST_FRAME_GRANTOR_REVOKED,
            TRUST_FRAME_SCOPE_EXCEEDS_GRANTOR, TRUST_FRAME_NODES_PATTERN_INVALID,
            REVOKE_FRAME_INVALID, REVOKE_FRAME_UNAUTHORIZED_ISSUER,
            REVOKE_FRAME_SERIAL_MISMATCH, REVOKE_FRAME_REASON_UNKNOWN,
            ASSURANCE_MISMATCH, ASSURANCE_UNKNOWN,
            REPUTATION_ENTRY_INVALID, REPUTATION_LOG_UNREACHABLE,
            REPUTATION_GOSSIP_FORK, REPUTATION_GOSSIP_SIG_INVALID,
