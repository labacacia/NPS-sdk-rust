...
use super::oids::{
    oid_equals, EKU_AGENT_IDENTITY_OID, EKU_NODE_IDENTITY_OID, EXTENSION_EXTENDED_KEY_USAGE_OID,
...
fn check_leaf_eku(leaf: &X509Certificate) -> Option<VerifyResult> {
    ...
    Some(fail(
        error_codes::CERT_EKU_MISSING,
        "leaf has no ExtendedKeyUsage extension",
    ))
}
        ...
        if oid_equals(&ext.oid, NID_ASSURANCE_LEVEL_OID) {
            ...
            if ext.value.len() != 3 || ext.value[0] != 0x0A || ext.value[1] != 0x01 {
                return Some(fail(
                    error_codes::CERT_FORMAT_INVALID,
                    format!(
                        "malformed assurance-level extension: {}",
            ...
            if cert_level != *asserted {
                return Some(fail(
                    ...
                    format!(
                        "cert assurance-level ({}) does not match asserted ({})",
...
fn verify_signed_by(child: &X509Certificate, issuer: &X509Certificate) -> Result<(), String> {
    ...
    if pub_key_bytes.len() != 32 {
        return Err(format!(
            "issuer pubkey is not 32 bytes ({} bytes)",
            pub_key_bytes.len()
        ));
    }
    let mut pk = [0u8; 32];
    ...
    let verifying_key =
        VerifyingKey::from_bytes(&pk).map_err(|e| format!("issuer Ed25519 pubkey parse: {e}"))?;
    ...
    verifying_key
        .verify(child.tbs_certificate.as_ref(), &signature)
...
fn raw_seq_contains_oid(value: &[u8], expected: &[u64]) -> bool {
    ...
    while pos + 2 <= end {
        ...
        if oid_start + oid_len > value.len() {
            return false;
        }
