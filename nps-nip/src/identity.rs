...
impl NipIdentity {
    ...
    pub fn save(&self, path: &Path, passphrase: &str) -> NpsResult<()> {
        ...
        let cipher =
            Aes256Gcm::new_from_slice(&key).map_err(|e| NpsError::Identity(e.to_string()))?;
        ...
        std::fs::write(path, envelope.to_string()).map_err(|e| NpsError::Io(e.to_string()))
    }

    /// Load from encrypted file.
    pub fn load(path: &Path, passphrase: &str) -> NpsResult<Self> {
        let data = std::fs::read_to_string(path).map_err(|e| NpsError::Io(e.to_string()))?;
        let v: serde_json::Value =
            serde_json::from_str(&data).map_err(|e| NpsError::Codec(e.to_string()))?;

        let salt_hex = v["salt"]
            .as_str()
            .ok_or(NpsError::Identity("missing salt".into()))?;
        let nonce_hex = v["nonce"]
            .as_str()
            .ok_or(NpsError::Identity("missing nonce".into()))?;
        let ct_hex = v["ciphertext"]
            .as_str()
            .ok_or(NpsError::Identity("missing ciphertext".into()))?;
