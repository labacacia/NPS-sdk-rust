...
mod tests {
    ...
    #[test]
    fn auth_nid_errors() {
        assert_eq!(to_nps_status(AUTH_NID_SCOPE_VIOLATION),    "NPS-AUTH-FORBIDDEN");
        ...
        assert_eq!(to_nps_status(AUTH_NID_UNTRUSTED_ISSUER),   "NPS-AUTH-UNAUTHENTICATED");
        assert_eq!(to_nps_status(AUTH_NID_CAPABILITY_MISSING), "NPS-AUTH-FORBIDDEN");
        assert_eq!(to_nps_status(AUTH_ASSURANCE_TOO_LOW),      "NPS-AUTH-FORBIDDEN");
    }

    #[test]
    fn reputation_errors() {
        assert_eq!(to_nps_status(REPUTATION_THROTTLED), "NPS-CLIENT-RATE-LIMITED");
    ...
    fn query_errors() {
        ...
        assert_eq!(to_nps_status(QUERY_VECTOR_UNSUPPORTED),     "NPS-SERVER-UNSUPPORTED");
        assert_eq!(to_nps_status(QUERY_AGGREGATE_UNSUPPORTED),  "NPS-SERVER-UNSUPPORTED");
        assert_eq!(to_nps_status(QUERY_AGGREGATE_INVALID),      "NPS-CLIENT-BAD-PARAM");
        assert_eq!(to_nps_status(QUERY_STREAM_UNSUPPORTED),     "NPS-SERVER-UNSUPPORTED");
    }

    #[test]
    fn action_and_task_errors() {
        assert_eq!(to_nps_status(ACTION_NOT_FOUND),             "NPS-CLIENT-NOT-FOUND");
        assert_eq!(to_nps_status(ACTION_PARAMS_INVALID),        "NPS-CLIENT-UNPROCESSABLE");
        assert_eq!(to_nps_status(ACTION_IDEMPOTENCY_CONFLICT),  "NPS-CLIENT-CONFLICT");
    ...
    #[test]
    fn subscribe_errors() {
        assert_eq!(to_nps_status(SUBSCRIBE_STREAM_NOT_FOUND),   "NPS-CLIENT-NOT-FOUND");
        assert_eq!(to_nps_status(SUBSCRIBE_LIMIT_EXCEEDED),     "NPS-LIMIT-EXCEEDED");
        assert_eq!(to_nps_status(SUBSCRIBE_FILTER_UNSUPPORTED), "NPS-SERVER-UNSUPPORTED");
        assert_eq!(to_nps_status(SUBSCRIBE_INTERRUPTED),        "NPS-SERVER-UNAVAILABLE");
    ...
    fn budget_and_limit_errors() {
        ...
        assert_eq!(to_nps_status(CGN_LIMIT_EXCEEDED),   "NPS-CLIENT-REQUEST-TOO-LARGE");
    ...
    fn manifest_and_topology_errors() {
        assert_eq!(to_nps_status(MANIFEST_VERSION_UNSUPPORTED),  "NPS-CLIENT-BAD-PARAM");
        assert_eq!(to_nps_status(MANIFEST_NODE_TYPE_REMOVED),    "NPS-CLIENT-BAD-FRAME");
        assert_eq!(to_nps_status(MANIFEST_NODE_TYPE_UNKNOWN),    "NPS-CLIENT-BAD-FRAME");
        assert_eq!(to_nps_status(TOPOLOGY_UNAUTHORIZED),         "NPS-AUTH-FORBIDDEN");
        assert_eq!(to_nps_status(TOPOLOGY_UNSUPPORTED_SCOPE),    "NPS-CLIENT-BAD-PARAM");
    }
    ...
    fn constants_have_correct_prefix() {
        let codes = [
            AUTH_NID_SCOPE_VIOLATION, AUTH_NID_EXPIRED, AUTH_NID_REVOKED,
            AUTH_NID_UNTRUSTED_ISSUER, AUTH_NID_CAPABILITY_MISSING,
            AUTH_ASSURANCE_TOO_LOW, AUTH_REPUTATION_BLOCKED,
            REPUTATION_THROTTLED, REPUTATION_REJECTED, REPUTATION_BANNED,
            QUERY_FILTER_INVALID, QUERY_FIELD_UNKNOWN, QUERY_CURSOR_INVALID,
            QUERY_REGEX_UNSAFE, QUERY_VECTOR_UNSUPPORTED, QUERY_AGGREGATE_UNSUPPORTED,
            QUERY_AGGREGATE_INVALID, QUERY_STREAM_UNSUPPORTED,
            ACTION_NOT_FOUND, ACTION_PARAMS_INVALID, ACTION_IDEMPOTENCY_CONFLICT,
            TASK_NOT_FOUND, TASK_ALREADY_CANCELLED, TASK_ALREADY_COMPLETED, TASK_ALREADY_FAILED,
            SUBSCRIBE_STREAM_NOT_FOUND, SUBSCRIBE_LIMIT_EXCEEDED,
            SUBSCRIBE_FILTER_UNSUPPORTED, SUBSCRIBE_INTERRUPTED, SUBSCRIBE_SEQ_TOO_OLD,
            BUDGET_EXCEEDED, CGN_LIMIT_EXCEEDED, DEPTH_EXCEEDED, GRAPH_CYCLE,
            NODE_UNAVAILABLE,
            MANIFEST_VERSION_UNSUPPORTED, MANIFEST_NODE_TYPE_REMOVED, MANIFEST_NODE_TYPE_UNKNOWN,
            RATE_LIMIT_EXCEEDED, RESERVED_TYPE_UNSUPPORTED,
            TOPOLOGY_UNAUTHORIZED, TOPOLOGY_UNSUPPORTED_SCOPE,
            TOPOLOGY_DEPTH_UNSUPPORTED, TOPOLOGY_FILTER_UNSUPPORTED,
        ];
        for c in &codes {
