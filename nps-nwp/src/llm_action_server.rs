...
use crate::llm::{
    LlmCompleteActionRequest, LlmCompleteActionResponse, LlmContextOperation,
    LlmContextReleaseRequestDto, LlmContextStatusRequestDto, LlmMessageDto, LlmStopReason,
    CAPABILITY_LLM_COMPLETE, CAPABILITY_LLM_CONTEXT, LLM_COMPLETE, LLM_COMPLETE_RESPONSE_ANCHOR,
    LLM_CONTEXT_RELEASE, LLM_CONTEXT_RELEASE_RESPONSE_ANCHOR, LLM_CONTEXT_STATUS,
...
pub type LlmContextAuthorizer = Arc<
    dyn Fn(&LlmContextOwner, &str, LlmAuthorizationStage, &ActionContext) -> Result<(), ActionError>
        + Send
        + Sync,
>;
...
pub struct StatefulLlmActionOptions {
    ...
    /// Optional NIP/capability check. Called at admission and again before commit.
...
impl StatefulLlmActionProvider {
    ...
    fn check_authorization(
        ...
        if let Some(authorizer) = &self.options.authorizer {
            authorizer(owner, action, stage, context)?;
        }
        Ok(())
    }
    ...
    fn complete(
        ...
        self.execute_reserved(frame, context, &owner, guard)
    }
    ...
    fn execute_reserved(
        ...
        if let Err(error) =
            self.check_authorization(owner, LLM_COMPLETE, LlmAuthorizationStage::Commit, context)
        {
            guard.abort(&error.error_code);
            return Err(error);
        }
