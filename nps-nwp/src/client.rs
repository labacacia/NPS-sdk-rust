...
impl NwpClient {
    ...
    pub async fn send_anchor(&self, frame: &AnchorFrame) -> NpsResult<()> {
        let wire =
            self.codec
                .encode(AnchorFrame::frame_type(), &frame.to_dict(), self.tier, true)?;
        let res = self
            .post(&format!("{}/anchor", self.base_url), wire)
            .await?;
    ...
    pub async fn query(&self, frame: &QueryFrame) -> NpsResult<CapsFrame> {
        let wire =
            self.codec
                .encode(QueryFrame::frame_type(), &frame.to_dict(), self.tier, true)?;
    ...
    pub async fn stream(&self, frame: &QueryFrame) -> NpsResult<Vec<StreamFrame>> {
        let wire =
            self.codec
                .encode(QueryFrame::frame_type(), &frame.to_dict(), self.tier, true)?;
        let res = self
            .post(&format!("{}/stream", self.base_url), wire)
            .await?;
    ...
    pub async fn invoke(&self, frame: &ActionFrame) -> NpsResult<InvokeResult> {
        let wire =
            self.codec
                .encode(ActionFrame::frame_type(), &frame.to_dict(), self.tier, true)?;
        let res = self
            .post(&format!("{}/invoke", self.base_url), wire)
            .await?;
