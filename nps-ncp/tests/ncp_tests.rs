...
fn hello_frame_full_roundtrip_json() {
    ...
    let wire = codec
        .encode(FrameType::Hello, &dict, EncodingTier::Json, true)
        .unwrap();
    ...
    assert_eq!(
        out.agent_id.as_deref(),
        Some("urn:nps:agent:example.com:hello-1")
    );
...
fn hello_frame_minimal_roundtrip_msgpack() {
    ...
    assert_eq!(
        out.max_concurrent_streams,
        HelloFrame::DEFAULT_MAX_CONCURRENT_STREAMS
    );
}
